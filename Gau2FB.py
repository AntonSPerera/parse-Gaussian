...
Parsing:
    Force   --> Looks for the keyword 'Forces (Hartrees/Bohr)'
    Energy  --> Looks for one of these keyword
                    BSSE : associated with a Counterpoise calculation
                           stands for Basis Set Superposition Error corrected
                           energy
                     SCF : SCF value
            Counterpoise : complexation energy of a Counterpoise
                           calculation, in kcal/mol
                   ONIOM : result of an ONIOM calculation
    Coordinate

Writing:

    it writes two different files, qdata.txt and all.gro

    qdata.txt:
        JOB    &NJOB
        COORDS &Coordinate [Angstrom]
        ENERGY &Energy [Hartree]
        FORCES &Force [Hartree/Bohr]
        ---
        JOB    &NJOB + 1
        ...

    all.gro:
        Title : Coordinate from &Namefile
           &Natom

           1SOL  &NameAtom   &IndexAtom  &CoordX, &CoordY, &CoordZ
           ...
              BoxX    BoxY    BOXZ
        ---
        Title : Coordinate from &Namefile + 1
        ...
...
def Gau_Forcebalance(filepath, ener_flag, job, name_file = ['qdata.txt', 'all.gro']):
    """
        ...
        Energy, Forces and Coordinate from the log file
        and writes the qdata.txt, all.gro files necessary to the
        ForceBalance procedure of fitting to qm data exploiting GROMACS

        Inputs
            filepath  : path to the file to be parsed
            name_file : ['qdata.txt, all.gro'] predefined named for the FB
                        procedure
            ener_flag : flag of the possible Gaussian calculation
            job        : Counter
    ...
    # Check if the proper energy flag has been used
    ...
    if os.path.isfile(filepath) == False:
        ...
        with open(name_file[1], 'a') as allgro:
            ...
            for i, _gro in enumerate(gro):
                if i == 0:
                    allgro.write("    1BrM {:>4}{:>4}    {:.4f}    {:.4f}     {:.4f}\n".format(
                    ...
                    allgro.write("{:4}SOL {:>4}{:>4}    {:.4f}    {:.4f}     {:.4f}\n".format(
                                                 count_mol,
                                                 _gro[0].replace('O','OW').replace('H','HW'),
                                                 conta_atom,
                                                 _gro[1]*0.1,
...
def walk_Gau_Forcebalance(path, ener_flag):
    """
        Function which walks in a directory looking for
        .log files and calling the Gau_Forcebalance Function
        to parse the file into one

        Inputs:
            path      : directory path
            ener_flag : energy flag
...
def dir_path(string):
    """
        Function which takes a string as input and check if
        is a path to a irectory or not

        Inputs:
            string : path to a directory

        Returns:
            string or raise a NotADirectoryError
    """
...
def main():
    ...
    parser.add_argument('--path',
                        type=dir_path,
                        help = "Insert the path to the in which are \
                                stored the Gaussian output files")
    parser.add_argument('--ener_flag',
                        help= "Insert a flag for the value of the energy \
                                to be parsed between:\n ONIOM, BSSE, SCF \
                                Counterpoise")
